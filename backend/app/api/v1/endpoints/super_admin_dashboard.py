...
async def export_overview(
    ...
    Includes metadata: timeframe, generated_at, filters
    """
    try:
        ...
        if format == "pdf":
            ...
            return Response(
                ...
                headers={"Content-Disposition": f"attachment; filename=overview_export_{timestamp}.pdf"}
            )
        else:
            # CSV export
            ...
            return StreamingResponse(
                ...
                media_type="text/csv",
                headers={"Content-Disposition": f"attachment; filename=overview_export_{timestamp}.csv"}
            )
    except Exception as e:
