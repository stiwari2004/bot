"""
Settings — execution mode and LLM budget endpoints
"""
...
async def get_execution_mode(
    ...
    try:
        ...
        return ExecutionModeResponse(mode=mode, description=_mode_description(mode))
...
async def set_execution_mode(
    ...
    try:
        ...
        return ExecutionModeResponse(mode=request.mode, description=_mode_description(request.mode))
...
async def get_execution_mode_demo(db: Session = Depends(get_db)):
    ...
    try:
        mode = ConfigService.get_execution_mode(db, 1)
        return ExecutionModeResponse(mode=mode, description=_mode_description(mode))
    except Exception as e:
...
async def set_execution_mode_demo(
    ...
    try:
        ConfigService.set_execution_mode(db, 1, request.mode)
        return ExecutionModeResponse(mode=request.mode, description=_mode_description(request.mode))
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
...
async def _build_budget_response(tenant_id: int) -> LLMBudgetPolicyResponse:
...
async def get_llm_budget_policy(
    ...
    return await _build_budget_response(tenant_id)
...
async def get_llm_budget_policy_demo():
    return await _build_budget_response(1)
