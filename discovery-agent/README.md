...
Then run **`python3 discover.py`** from that folder (Linux/macOS/Windows) so the same run includes all configured scanners. Alternatively use `python run.py config.yaml`.
