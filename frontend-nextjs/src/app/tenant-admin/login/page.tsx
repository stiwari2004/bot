...
export default function MspAdminLoginPage() {
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [error, setError] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);
  const { login } = useMspAdminAuth();
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-xl border border-neutral-200 p-8">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-neutral-900 mb-2">MSP Admin Portal</h1>
            <p className="text-sm text-neutral-600">White-label administration portal</p>
          </div>
          ...
          <div className="mt-6 text-center">
            <p className="text-xs text-neutral-500">
              This is a secure MSP administration area
            </p>
          </div>
        </div>
      </div>
