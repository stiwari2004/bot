#!/bin/bash
# Migration script for DEV environment
# Run this from the project root directory

CONTAINER_NAME="bot-dev-postgres"
DB_NAME="troubleshooting_ai_dev"

echo "=== Running Migration for DEV Environment ==="
echo "Container: $CONTAINER_NAME"
echo "Database: $DB_NAME"
echo ""

# Check if SQL file exists
if [ -f "backend/sql/create_scheduled_reports_table.sql" ]; then
    echo "✅ SQL file found, copying to container..."
    docker cp backend/sql/create_scheduled_reports_table.sql $CONTAINER_NAME:/tmp/create_scheduled_reports_table.sql
    docker exec -i $CONTAINER_NAME psql -U postgres -d $DB_NAME -f /tmp/create_scheduled_reports_table.sql
    echo "✅ Migration completed!"
else
    echo "❌ SQL file not found on server"
    echo "Running migration directly from SQL content..."
...
docker exec -i $CONTAINER_NAME psql -U postgres -d $DB_NAME -c "\d scheduled_reports"
echo ""
docker exec -i $CONTAINER_NAME psql -U postgres -d $DB_NAME -c "\dT+ reportfrequency"
